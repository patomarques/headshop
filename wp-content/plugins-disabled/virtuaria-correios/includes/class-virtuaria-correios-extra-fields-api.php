<?php
/**
 * Handle Extra Fields REST API integration.
 *
...
/**
 * Handle fields in REST API.
 */
class Virtuaria_Correios_Extra_Fields_API {
	/**
	...
	protected function get_person_type( $type ) {
		switch ( isset( $this->fields_settings['person_type'] ) ) {
			case 'both':
				$persontype = 'pj' === $type
					? 'J'
					: 'F';
	...
	public function legacy_customers_response( $customer_data, $customer, $fields, $server ) {
		// Billing fields.
		$customer_data['billing_address']['persontype']   = $this->get_person_type( $customer->get_meta( '_billing_persontype' ) );
		$customer_data['billing_address']['cpf']          = $this->format_number( $customer->get_meta( '_billing_cpf' ) );
		$customer_data['billing_address']['rg']           = $this->format_number( $customer->get_meta( '_billing_rg' ) );
		$customer_data['billing_address']['cnpj']         = $this->format_number( $customer->get_meta( '_billing_cnpj' ) );
		$customer_data['billing_address']['ie']           = $this->format_number( $customer->get_meta( '_billing_ie' ) );
		$customer_data['billing_address']['birthdate']    = $this->get_formatted_birthdate_legacy( $customer->get_meta( '_billing_birthdate' ), $server );
		$customer_data['billing_address']['gender']       = substr( $customer->get_meta( '_billing_gender' ), 0, 1 );
		$customer_data['billing_address']['number']       = $customer->get_meta( '_billing_number' );
		$customer_data['billing_address']['neighborhood'] = $customer->get_meta( '_billing_neighborhood' );
		$customer_data['billing_address']['cellphone']    = $customer->get_meta( '_billing_cellphone' );
	...
	public function customers_response( $response, $user ) {
		...
		// Billing fields.
		$response->data['billing']['number']       = $customer->get_meta( '_billing_number' );
		$response->data['billing']['neighborhood'] = $customer->get_meta( '_billing_neighborhood' );
		$response->data['billing']['persontype']   = $this->get_person_type( $customer->get_meta( '_billing_persontype' ) );
		$response->data['billing']['cpf']          = $this->format_number( $customer->get_meta( '_billing_cpf' ) );
		$response->data['billing']['rg']           = $this->format_number( $customer->get_meta( '_billing_rg' ) );
		$response->data['billing']['cnpj']         = $this->format_number( $customer->get_meta( '_billing_cnpj' ) );
		$response->data['billing']['ie']           = $this->format_number( $customer->get_meta( '_billing_ie' ) );
		$response->data['billing']['birthdate']    = $this->get_formatted_birthdate( $customer->get_meta( '_billing_birthdate' ) );
		$response->data['billing']['gender']       = substr( $customer->get_meta( '_billing_gender' ), 0, 1 );
		$response->data['billing']['cellphone']    = $customer->get_meta( '_billing_cellphone' );

		// Shipping fields.
		$response->data['shipping']['number']       = $customer->get_meta( '_shipping_number' );
		$response->data['shipping']['neighborhood'] = $customer->get_meta( '_shipping_neighborhood' );
	...
	public function addresses_schema( $properties ) {
		$properties['billing']['properties']['number']        = array(
			'description' => __( 'Number.', 'virtuaria-correios' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
		);
		$properties['billing']['properties']['neighborhood']  = array(
			'description' => __( 'Neighborhood.', 'virtuaria-correios' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
		);
		$properties['billing']['properties']['persontype']    = array(
			'description' => __( 'Person type.', 'virtuaria-correios' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
		);
		$properties['billing']['properties']['cpf']           = array(
			'description' => __( 'CPF.', 'virtuaria-correios' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
		);
		$properties['billing']['properties']['rg']            = array(
			'description' => __( 'RG.', 'virtuaria-correios' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
		);
		$properties['billing']['properties']['cnpj']          = array(
			'description' => __( 'CNPJ.', 'virtuaria-correios' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
		);
		$properties['billing']['properties']['ie']            = array(
			'description' => __( 'IE.', 'virtuaria-correios' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
		);
		$properties['billing']['properties']['birthdate']     = array(
			'description' => __( 'Birthdate.', 'virtuaria-correios' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
		);
		$properties['billing']['properties']['gender']        = array(
			'description' => __( 'Gender.', 'virtuaria-correios' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
		);
		$properties['billing']['properties']['cellphone']     = array(
			'description' => __( 'Cell Phone.', 'virtuaria-correios' ),
			'type'        => 'string',
			'context'     => array( 'view', 'edit' ),
		);
